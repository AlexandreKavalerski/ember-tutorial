...
module('Integration | Component | ak-map', function (hooks) {
  setupRenderingTest(hooks);

  test('it renders a map image for the specified parameters', async function (assert) {
    await render(hbs`<AkMap
      @lat="37.7797"
      @lng="-122.4184"
      @zoom="10"
      @width="150"
    ...
    let token = encodeURIComponent(ENV.MAPBOX_ACESS_TOKEN);
    ...
    assert.ok(
      ...
      'the src should include the escaped acess token'
    );
  });
